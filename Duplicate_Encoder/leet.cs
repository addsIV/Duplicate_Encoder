...
namespace Duplicate_Encoder
{
	internal class leet
	{
		public bool InterLeaveString(string s1, string s2, string s3)//leet 97
		{
			if (s1.Length + s2.Length != s3.Length) return false;

			//dp[i,j] means if s3[i + j] is the InterLeaveString of s1[i] and s2[j]
		...
		public class LeetCode_Solution
		{
			...
			public int BitwiseComplement(int n)//leet 1009
			{
				...
				char[] temp = binary_n.ToCharArray();
				...
				return Convert.ToInt32(new string(temp), 2);
			}
			...
			public IList<TreeNode> DFS(int start, int end)
			{
